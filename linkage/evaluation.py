...
def charger_evaluation(lignes: list[dict], environ: dict[str, str] | None = None) -> int:
    ...
    Les colonnes de grappe de la table (nb_grappes_predites,
    nb_grappes_exactes, nb_enregistrements_sur_fusionnes) n'ont, dans la
    DDL déjà écrite, aucune distinction de portée restreinte/globale :
    elles sont peuplées avec la portée GLOBALE (sur l'ensemble de la
    population), la plus complète des deux — la portée restreinte reste
    disponible séparément, calculée par ce module, jamais absente de
    l'analyse, seulement absente de cette table précise.
    """
    valeurs = [
        (
            ligne["seuil"],
        ...
        curseur.executemany(
            ...
            insert into linkage.evaluation
                (seuil, vrais_positifs, faux_positifs, faux_negatifs,
                 ...
                 nb_grappes_predites, nb_grappes_exactes,
                 nb_enregistrements_sur_fusionnes, nb_paires_verite_terrain)
...
def main() -> None:
    ...
    grille = sorted(
        set(grille_seuils_requise()) | set(grille_seuils_etendue(max(poids_non_vt), min(poids_vt)))
    )
