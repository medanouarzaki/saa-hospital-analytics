...
%   OBS: REL-IPP.O03, REL-IPP.R12, REL-PAT.H12, REL-PAT.H14
%   HYP: absentéisme-par-spécialité, ancienneté-des-créances, complétude-par-champ, répartition-des-niveaux-de-tri
%   CHI: (aucun)
%   SER: (aucune)
%   SECTIONS: 5
...
\textbf{Ce qui établit le constat} vient toujours d'une source extérieure au jeu de données — un
rapport de contrôle, une publication statistique, une observation au poste. Une valeur affichée par
le tableau de bord ne le peut pas : les relations qui la gouvernent y ont été injectées.

\textbf{Ce que l'indicateur démontre} est autre chose : que la chaîne sait produire le nombre, à la
maille où une décision se prend. Aujourd'hui c'est une capacité ; branchée sur les données réelles,
ce sera un constat.

Aucun effort n'est chiffré et aucun effet n'est quantifié.
...
\textbf{Elle suppose surtout un numéro juste, et c'est là que la difficulté se trouve.} Le
chapitre~\ref{chap:systeme} relève que le champ est parfois rempli d'un numéro inventé lorsque la
personne ne connaît pas le sien. Un rappel envoyé sur un tel numéro ne revient pas en erreur : il
part, et personne ne le reçoit. La première mesure à prendre n'est donc pas l'envoi, mais le taux
d'aboutissement des envois — c'est lui qui dira quelle part du fichier de contact est utilisable.

\textbf{L'effet attendu et sa mesure.} On attend une baisse du taux d'absence sur les activités
ciblées, et une réduction du délai médian par récupération des créneaux perdus. La mesure existe :
le taux d'absence et le délai d'obtention par activité, comparés avant et après. La distinction
entre absence et annulation permet de vérifier que la baisse vient bien des absences.
