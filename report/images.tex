...
% LA RACINE DES FIGURES, ET POURQUOI ELLE EXISTE. `\IfFileExists` cherche depuis le
% répertoire de composition, et il N'HONORE PAS `\graphicspath` : c'est mesuré, non
% supposé. Un document d'essai composé depuis `slides/`, portant `\graphicspath{{../report/}}`
% et appelant `\logosInstitutionnels`, compose les deux CADRES D'ATTENTE et non les
% logotypes — la recherche de fichier échoue là où l'inclusion aurait réussi.
%
% La racine est donc déclarée ici, vide, et le rapport ne s'en aperçoit pas. Un document
% composé depuis un autre répertoire la redéfinit et réemploie la commande telle quelle,
% plutôt que d'en écrire une seconde qui divergerait.
\newcommand{\racineFigures}{}
...
\newcommand{\unLogo}[5]{%
  \raisebox{\dimexpr-0.5\height-#4\relax}{%
    \IfFileExists{\racineFigures figures/logos/#1.pdf}{%
      \includegraphics[height=#2]{\racineFigures figures/logos/#1.pdf}%
    }{%
      \IfFileExists{\racineFigures figures/logos/#1.png}{%
        \includegraphics[height=#2]{\racineFigures figures/logos/#1.png}%
      }{%
...
% LA CAPTURE ET SA LÉGENDE NE SE SÉPARENT PAS. Elles étaient deux blocs successifs, qu'une
% coupure de page pouvait disjoindre : la capture de la page « Données » finissait une page et
% « Figure 2 — La page Données… » ouvrait la suivante, orpheline de l'image qu'elle nommait. Le
% `minipage` en fait un bloc unique, insécable par construction. Il peut laisser du blanc en bas
% d'une page quand l'ensemble n'y tient plus ; c'est le prix, et il est moindre.
