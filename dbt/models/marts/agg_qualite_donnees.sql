...
quarantaine as (
    select
        'int_creances' as nom_table,
        count(*) as lignes_quarantaine
    from quarantaine.creances
    union all
    select
        'int_encaissements' as nom_table,
        count(*) as lignes_quarantaine
    from quarantaine.encaissements
    union all
    select
        'int_factures' as nom_table,
        count(*) as lignes_quarantaine
    from quarantaine.factures
    union all
    select
        'int_lignes_facture' as nom_table,
        count(*) as lignes_quarantaine
    from quarantaine.lignes_facture
    union all
    select
        'int_mouvements' as nom_table,
        count(*) as lignes_quarantaine
    from quarantaine.mouvements
    union all
    select
        'int_passages' as nom_table,
        count(*) as lignes_quarantaine
    from quarantaine.passages
    union all
    select
        'int_passages_urgences' as nom_table,
        count(*) as lignes_quarantaine
    from quarantaine.passages_urgences
    union all
    select
        'int_patients' as nom_table,
        count(*) as lignes_quarantaine
    from quarantaine.patients
    union all
    select
        'int_prises_en_charge' as nom_table,
        count(*) as lignes_quarantaine
    from quarantaine.prises_en_charge
    union all
    select
        'int_relances' as nom_table,
        count(*) as lignes_quarantaine
    from quarantaine.relances
    union all
    select
        'int_rendez_vous' as nom_table,
        count(*) as lignes_quarantaine
    from quarantaine.rendez_vous
)
